...
type DirectExecutor struct {
	*parser.BaseInfraDSLListener
	...
	devopsUtils   *executor.DevOpsUtils
	useSSH        bool
}
...
func (e *DirectExecutor) SetVerbose(v bool) {
	e.verbose = v
	if e.sshExecutor != nil {
		e.sshExecutor.SetVerbose(v)
	}
}
...
func (e *DirectExecutor) CheckSSHConnectivity(target string) error {
	...
	// For Docker testing (localhost:2222), skip SSH connectivity check
	// and let the SSH executor handle authentication
	if strings.Contains(target, "localhost:2222") {
		if e.verbose {
			utils.PrintInfo("Docker testing detected - skipping SSH connectivity check")
		}
		return nil
	}

	// Check if target includes port (for other remote servers)
	var cmd *exec.Cmd
...
func (e *DirectExecutor) Execute() error {
	...
	// Try to enable SSH if target is specified
	target := e.GetTarget()
	if target != "" && !e.useSSH {
		if e.verbose {
			utils.PrintInfo(fmt.Sprintf("Attempting to enable SSH for target: %s", target))
		}
		...
		if len(parts) == 2 {
			...
			// For Docker testing, try SSH keys first, then password
			if strings.Contains(hostPort, "localhost:2222") {
				if e.verbose {
					utils.PrintInfo("Docker testing detected - trying SSH key authentication")
				}
				
				// Try SSH agent first (this should work since you used ssh-copy-id)
				err := e.EnableSSH(hostPort, user, "", "")
				if err != nil {
					if e.verbose {
						utils.PrintInfo(fmt.Sprintf("SSH agent failed, trying SSH key: %v", err))
					}
					
					// Try current user's SSH key (since you're running SCL from your machine)
					currentUser := os.Getenv("USER")
					if currentUser == "" {
						currentUser = os.Getenv("USERNAME") // Windows fallback
					}
					
					homeDir := "/Users/" + currentUser  // macOS path
					keyPath := homeDir + "/.ssh/id_rsa"
					
					err = e.EnableSSH(hostPort, user, keyPath, "")
					if err != nil {
						if e.verbose {
							utils.PrintInfo(fmt.Sprintf("SSH key failed, trying password: %v", err))
						}
						
						// Fallback to password authentication
						err = e.EnableSSHWithPassword(hostPort, user, "ubuntu123")
						if err != nil {
							utils.PrintInfo(fmt.Sprintf("All SSH methods failed: %v", err))
						}
					}
				}
			} else {
				// For other servers, try SSH agent first, then key-based auth
				// Try SSH agent first
				err := e.EnableSSH(hostPort, user, "", "")
				if err != nil {
					if e.verbose {
						utils.PrintInfo(fmt.Sprintf("SSH agent failed, trying default key: %v", err))
					}
					// Try default SSH key
					err = e.EnableSSH(hostPort, user, fmt.Sprintf("/home/%s/.ssh/id_rsa", user), "")
					if err != nil {
						utils.PrintInfo(fmt.Sprintf("SSH connection failed, falling back to direct execution: %v", err))
					}
				}
			}
		}
...
func (e *DirectExecutor) executeCheckCommand(varName, expr string) bool {
	if e.useSSH {
		// Use SSH executor for remote checks
		_, err := e.sshExecutor.RunWithTimeout("command -v curl && command -v wget", 10*time.Second)
		result := err == nil
		
		if e.verbose {
			utils.PrintInfo(fmt.Sprintf("Remote check result for %s: %t", varName, result))
		}
...
func (e *DirectExecutor) executeCopyCommand(argList parser.IArgumentListContext) error {
	...
	if e.useSSH {
		return e.sshExecutor.FileOperations("upload", source, dest)
	}
...
func (e *DirectExecutor) executeCreateCommand(argList parser.IArgumentListContext) error {
	...
	// Check if we have a target (remote) or should execute locally
	target := e.variables["target"]
	
	if target == "" {
		// Local execution
		if e.verbose {
			utils.PrintInfo(fmt.Sprintf("Creating file %s with mode %s locally", fullPath, mode))
		}
		...
		output, err := cmd.CombinedOutput()
		if err != nil {
			errorMsg := fmt.Sprintf("local file creation failed: %v", err)
			if len(output) > 0 {
				errorMsg += fmt.Sprintf("\nCommand output: %s", string(output))
			}
			return fmt.Errorf(errorMsg)
		...
		utils.PrintSuccess(fmt.Sprintf("✓ Created file %s with mode %s locally", fullPath, mode))
		if e.verbose && len(output) > 0 {
			utils.PrintInfo(fmt.Sprintf("Command output: %s", string(output)))
		}
		return nil
	...
	if e.useSSH {
		// Use a single command to create directory and file
		createCmd := fmt.Sprintf("mkdir -p %s && touch %s && chmod %s %s", destination, fullPath, mode, fullPath)
		
		if e.verbose {
			utils.PrintInfo(fmt.Sprintf("Executing SSH command: %s", createCmd))
		}
		
		_, err := e.sshExecutor.RunWithTimeout(createCmd, 10*time.Second)
		if err != nil {
			return fmt.Errorf("failed to create file via SSH: %v", err)
		}
		
		utils.PrintSuccess(fmt.Sprintf("✓ Created file %s with mode %s via SSH", fullPath, mode))
		return nil
	}

	// Fallback to SSH command execution
	if e.verbose {
...
func (e *DirectExecutor) executeInstallCommand(argList parser.IArgumentListContext) error {
	...
	if len(parts) >= 2 {
		...
		if e.useSSH {
			return e.sshExecutor.PackageManagement("install", pkg)
		}
